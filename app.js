...
// --- PDF: usa logo, colores y formato DMYC ---
...
function drawPdfContent(doc, q, formatMoney, logo) {
    ...
    doc.setFontSize(11);
   // Bloque central (más cercano al logo)
doc.text("DMYC spa 76.935.323-2", 70, 15);
doc.setFont("helvetica", "normal");
doc.setFontSize(9);
doc.text("Cerro el plomo 5931 of 1213, Las Condes", 70, 20);
doc.text("Región Metropolitana", 70, 25);

// Bloque derecho (más hacia el margen)
doc.setFontSize(9);
doc.setFont("helvetica", "bold");
doc.text("COTIZACIÓN N°", 150, 15);
doc.setFont("helvetica", "normal");
doc.text(q.id, 185, 15);

doc.setFont("helvetica", "bold");
doc.text("FECHA", 150, 20);
doc.setFont("helvetica", "normal");
doc.text(q.date, 178, 20);

doc.setFont("helvetica", "bold");
doc.text("VÁLIDO HASTA", 150, 29);
doc.setFont("helvetica", "normal");
doc.text(q.validDate || q.date, 182, 29);
